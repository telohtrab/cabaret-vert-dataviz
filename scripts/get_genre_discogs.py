...
def search_genre(artist_id):
    ...
    releases = sorted(
    [r for r in response.get('releases', []) if r.get('type') == 'master'],
        key=lambda r: r.get('year', 0),
        reverse=True
    )[:5] # Limit to first 5 master releases to avoid too many API calls
    if not releases:
        releases = sorted(
             [r for r in response.get('releases', []) if r.get('type') == 'release'],
            key=lambda r: r.get('year', 0),
            reverse=True
    )[:5] # If no master releases, take first 5 releases
    ...
    for r in releases:  
        ...
        if master_id:
            ...
            time.sleep(2)  # Respect Discogs rate limit (60 requests/minute)
...
with open('data/raw/uniqueartists_backup.csv', 'r', encoding='utf-8') as csvfile:
    with open('data/raw/uniqueartists_new.csv', 'w', encoding='utf-8', newline='') as outfile:
        ...
        for artist in artists:
            if artist['discogs_id'] and artist['discogs_id'] != 'x':
                genre, styles = search_genre(artist['discogs_id'])
                artist['genre'] = genre
